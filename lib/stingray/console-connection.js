...
export function createSocketStream(url) {
    ...
    const observable = Rx.Observable.create(o => {        
        ...
        const id = counts[url] = (counts[url] || 0) + 1;
        console.log(`Creating socket '${url}(${id})'`);
        ...
        socket.onclose = e => {
            console.log(`Closed socket '${url}(${id})'`);
            counts[url] = (counts[url] || 0) - 1;
            // CLOSE_NORMAL,  CLOSE_ABNORMAL or CLOSE_NO_STATUS
            if (!error && (e.code == 1000 || e.code == 1006 || e.code == 1005))
                o.onCompleted();
            else
        ...
        return () => {
            console.log(`Disposing socket '${url}(${id})'`);
            socket.close();
            socket = null;
        };
    });
...
export function isPortOpen(value, host, port) {
    return Rx.Observable.create(observer => {
        ...
        socket.setTimeout(50, () => socket.destroy());
...
export function send(url, o) {
    ...
    socket.delay(1000).subscribe(e => socket.onCompleted());
}
