...
export function sourceRepositoryPath(config) {
    if (config.SourceRepositoryPath === undefined)
        throw new Error("Toolchain doesn't have a source repository configured.");

    return config.SourceRepositoryPath;
}
...
export function assetServerExecutable(toolchainPath) {
    return path.join(toolchainPath, "engine", "win64", "dev", "stingray_win64_dev_x64.exe")
}

export function runtimeExecutable(toolchainPath) {
    return path.join(toolchainPath, "engine", "win64", "dev", "stingray_win64_dev_x64.exe")
}
