...
export function runningToolchainPath() {
    const runningPath = Rx.Observable.create(observer => {
        ...
        const subscription = connection
            ...
            .map(x => x.data && x.data.result && x.data.result.remoteObjects[0].value)
            .filter(x => x !== undefined && x.ToolChainDirectory)
            .subscribe(x => {
    ...
    return runningPath.timeout(100, preferredToolchainPath());
}

function preferredToolchainPath() {
    return Rx.Observable.create(observer => {
        var value = atom.config.get('stingray.toolchainPath') || process.env.SR_BIN_DIR;
        if (value) {
            observer.onNext(value);
            observer.onCompleted();
        } else {
            observer.onError("Stingray toolchain not found.");
        }
    });
}

// Path to tool chain configuration file.
export function runningToolchainConfigurationPath() {
    return runningToolchainPath().map(
            x => {
                var toolchainConfig = path.join(x, "settings", "ToolChainConfiguration.config");
                var userConfig = path.join(process.env.HOME, "AppData", "Local", "Autodesk", "Stingray", "Settings", "ToolChainConfiguration.config");
                if (fs.existsSync(toolchainConfig))
                    return toolchainConfig;
                else if (fs.existsSync(userConfig))
                    return userConfig;
                else
                    throw new Error("`ToolChainConfiguration.config` not found.");
            }
    );
}

const readFile = Rx.Observable.fromNodeCallback(fs.readFile);
export function configuration(toolchainConfigPath) {
    return readFile(toolchainConfigPath)
        .map(SJSON.parse)
