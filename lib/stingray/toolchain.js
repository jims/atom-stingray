...
export function runningToolchainPath() {
    const runningPath = Rx.Observable.create(observer => {
        ...
        const connection = createConsoleConnection(`ws://localhost:${EDITOR_CHANNEL_PORT}?namespaces=${EDITOR_MARSHALLING_NAMESPACE}`);
    ...
    return runningPath.timeout(200, Rx.Observable.return(preferredToolchainPath()));
}
