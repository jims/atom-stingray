...
function drawHistory(context, historyData, historyFrameTimes, width, height) {
    ...
    for (let i = length - 1; i >= 0 && x > -w; --i) {
        ...
        context.fillStyle = historyColors.background;
...
class Profiler {
    ...
    static deserialize(state) {
        ...
        profiler.lastData = state.lastData;
    ...
    _recvProfilerDataBuffer(buffer) {
        ...
        const range = getRange(this.lastData);
...
class ProfilerElementImpl extends HTMLElement {
    ...
    update() {
        ...
        drawHistory(this._history.getContext("2d"), profiler.historyData, profiler.historyFrameTimes, this._history.width, this._history.height);
    }
}
