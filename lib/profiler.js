...
class Profiler {
    constructor(url, socketObservable) {
        console.assert(typeof(url) === "object" && typeof(url.host) === "string");
        console.assert(socketObservable != null && typeof(socketObservable.onNext) === "function");
        this.offset = 0;
        this.scale = 1;
        ...
        this.lastData = null;
        this._url = url;
        this._socketObservable = socketObservable;
        this._allThreads = {};
    ...
    pause() {
        this._socketObservable.onNext(ConsoleConnection.encodeCommand("profiler pause"));
    }

    resume() {
        this._socketObservable.onNext(ConsoleConnection.encodeCommand("profiler unpause"));
    }
...
class ProfilerElementImpl extends HTMLElement {
    initialize(profiler) {
        ...
        pauseButton.classList.add("icon-playback-pause");
        pauseButton.onclick = event => this.pauseProfiling();
        ...
        canvasContainer.style.top = toolBarHeight + "px";
    ...
    windowOnResize() {
        ...
        const canvasContainer = this._canvasContainer;
...
export class ProfilingService {
    ...
    opener(uri) {
        ...
        if (url.protocol === "stingray-profiler:") {
            ...
            return new Profiler(url, socketObservable);
        }
    }
