...
const historyColors = Object.freeze({
    ...
    graphTimeRangeLabel: theme.lightForeground,
    overflowIndicator: "rgba(0, 0, 0, 0.2)",
...
function drawHistory(context, historyData, historyFrameTimes, viewedHistoryIndex, hoveredHistoryIndex, historyGraphTimeRange, width, height) {
    const length = historyData.length;
    const roundedtimeRange = Math.round(historyGraphTimeRange * 1000) / 1000;
    ...
    for (let i = length - 1; i >= 0 && x > -w; --i) {
        ...
        const percentage = frameTime / roundedtimeRange;
        const clampedPercentage = Math.min(percentage, 1.0);
        context.fillRect(0, x, height * clampedPercentage, w - 1);
        ...
        if (percentage > 1.0) {
            context.fillStyle = historyColors.overflowIndicator;
            context.beginPath();
            context.moveTo(height - 3, x - 0.5 + w / 2);
            context.lineTo(height - 10, x + w - 3.5);
            context.lineTo(height - 10, x + 2.5);
            context.closePath();
            context.fill();
        }
    ...
    context.restore();
    context.fillStyle = historyColors.graphTimeRangeLabel;
    const timeRange = (roundedtimeRange * 1000) + " ms";
    context.fillText(timeRange, 3, 10);
}
...
class Profiler {
    constructor(url) {
        ...
        this.historyGraphTimeRange = 50 / 1000;
    ...
    serialize() {
        ...
        const historyData = this.historyData.slice(-1);
        const historyFrameTimes = this.historyFrameTimes.slice(-1);
        return {
            ...
            historyFrameTimes: historyFrameTimes,
            historyGraphTimeRange: this.historyGraphTimeRange,
            viewedHistoryIndex: historyData.length - 1,
    ...
    static deserialize(state) {
        ...
        profiler.historyData = state.historyData;
        profiler.historyFrameTimes = state.historyFrameTimes;
        profiler.historyGraphTimeRange = state.historyGraphTimeRange;
        profiler.viewedHistoryIndex = state.viewedHistoryIndex;
    ...
    _recvProfilerDataBuffer(buffer) {
        ...
        const data = parseProfilerDataBuffer(buffer);
...
class ProfilerElementImpl extends HTMLElement {
    initialize(profiler) {
        ...
        history.addEventListener("wheel", this.historyOnMouseWheel.bind(this));
    ...
    historyOnMouseWheel(event) {
        const profiler = this._profiler;
        const oldRange = profiler.historyGraphTimeRange;
        const newRange = event.deltaY > 0 ? oldRange * 1.2 : oldRange / 1.2;
        profiler.historyGraphTimeRange = Math.min(Math.max(0.001, newRange), 1.0);
    }
    ...
    update() {
        ...
        const profiler = this._profiler;
        const viewedHistoryIndex = profiler.viewedHistoryIndex;
        const threadInfos = profiler.threadInfos;

        if (viewedHistoryIndex < 0 || !threadInfos)
            return;
        ...
        const viewedData = profiler.historyData[viewedHistoryIndex];
        ...
        drawHistory(this._history.getContext("2d"), profiler.historyData, profiler.historyFrameTimes, viewedHistoryIndex, this._hoveredHistoryIndex, profiler.historyGraphTimeRange, this._history.width, this._history.height);
    }
}
