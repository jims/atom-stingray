...
class StingrayPackage {
    constructor(state) {
        ...
        const idStringLookup = this.projects()
            .first()
        ...
        const consoleSubscription = localPorts
            ...
            .flatMap(url => ConsoleConnection.create(url)
                ...
                .combineLatest(idStringLookup, (engineMessage, lookup) => ({source: url, logEntry: LogEntry.fromEngineMessage(engineMessage, lookup)})))
            .subscribe(x => this.logConsole.addEntry(x.source, x.logEntry));

        this.disposables.add(idStringLookup);
