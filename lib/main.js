...
class StingrayPackage {
    constructor(state) {
        ...
        this.disposables.add(atom.commands.add("atom-workspace", "stingray:terminate-engine", this.terminateEngine.bind(this)));
        ...
        this.consoleConnections = localPorts
            ...
            .map(x => ({
                host: x.host,
                port: x.port,
                connection: ConsoleConnection.create(`ws://${x.host}:${x.port}`)
            }));

        const consoleSubscription = this.consoleConnections
            .subscribe(
                x => this.logConsole.handleEngineDetected(x.host, x.port, x.connection),
                e => {console.log(e);}
            );
            ...
            const subscription = this.projects()
                ...
                .subscribe(
                    event => {
                        ...
                        this.progressIndicator.active = false;
                        if (project != null) {
                            atom.notifications.addSuccess(`Compiled ${files.length} ${files.length === 1 ? "file" : "files"}.`);
    ...
    terminateEngine(event) {
        const assetServerExecutable = this.toolChainConfiguration.map(Toolchain.assetServerExecutable);
        const runtimeExecutable = this.toolChainConfiguration.map(Toolchain.runtimeExecutable);
        const engineExecutables = assetServerExecutable.concat(runtimeExecutable).map(x => path.basename(x)).toSet();
        const runningTasks = ProcessUtils.runningTasks();
        const runningEngineTasks = runningTasks.zip(engineExecutables, (tasks, exes) => tasks.filter(task => exes.has(task.name)));
        const processIds = runningEngineTasks.map(tasks => tasks.map(task => task.pid));

        processIds.subscribe(pids => {
            if (pids.length < 1)
                return;
            ...
            atom.notifications.addSuccess(`Terminated ${pids.length} Stingray ${pids.length === 1 ? "process" : "processes"}.`);
        });
    }
