...
class StingrayPackage {
    ...
    expandSelection(f) {
        ...
        te.setSelectedBufferRange(
            [[row, start], [row, end]]
        );
    }

    openResource(event) {
        this.expandSelection(s => s.match(/^[^'"`]*$/))

        var te = atom.workspace.getActiveTextEditor();
        var resource = te.getSelectedText();
    ...
    helpSelection(event) {
        this.expandSelection(s => s.match(/^[a-zA-Z._0-9]*$/));
        var s = Help.stripStingray(atom.workspace.getActiveTextEditor().getSelectedText());
        if (Help.has(s))
