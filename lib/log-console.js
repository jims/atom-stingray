...
function createTabDomElement(title) {
    console.assert(typeof(title) === "string")
    const tabElement = createDomElementWithClass("li", "tab");
    const titleElement = createDomElementWithClass("div", "title");
...
class LogConsoleTab {
    constructor(id, title, active, logEntries) {
    ...
    createDomElements(tabBarElement, listElement, handleMouseDown) {
        ...
        const tabElement = createTabDomElement(this._title);
...
export class LogConsole {
    ...
    handleEngineDetected(host, port) {
        ...
        if (tab == null) {
            // Create a new tab.
            const title = host + ":" + port;
            const active = this._tabs.length === 0;
            tab = new LogConsoleTab(id, title, active, []);
    ...
    set visible(value) {
        ...
        if (value) {
            ...
            this._tabs.forEach(tab => tab.createDomElements(tabBar, list));
