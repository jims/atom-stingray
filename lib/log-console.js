...
class LogConsoleTab {
    constructor(url, title, icon, active, first, logEntries, getElementsCallback) {
    ...
    set socketObservable(observable) {
        ...
        this._socketSubscription = observable
            ...
            .map(LogEntry.fromEngineMessage)
    ...
    addEntry(logEntry) {
        const {list} = this._getElementsCallback();
        console.assert(isLogEntry(logEntry));
...
export class LogConsole {
    ...
    handleEngineDetected(host, port, socketObservable) {
        ...
        if (tab == null) {
            ...
            tab = new LogConsoleTab(url, title, icon, active, first, [], () => this._elements);
        ...
        if (domElements != null) {
            for (let i = 0, count = this._tabs.length; i < count; ++i) {
                ...
                examinedTab.setActive(examinedTab === tab, domElements.tabBar, domElements.list);
            }
        }
    }
    ...
    handleCommandLineKeyDown(evt) {
        ...
        if (EventUtils.consumeIf(evt, x => x.keyCode === KeyCodes.upKey)) {
            ...
            if (text != "") {
                ...
                if (tab != null)
                    tab.socketObservable.onNext(JSON.stringify(ConsoleConnection.encodeLua(text)));
            }
        }
    };
