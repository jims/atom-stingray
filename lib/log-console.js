...
function createDomElements(handleCommandLineTextEntry) {
    console.assert(typeof(handleCommandLineTextEntry) === "function");
    ...
    const handleCommandLineKeyDown = evt => {
        ...
        if (evt.keyCode === 13) {
            ...
            if (text != "") {
                handleCommandLineTextEntry(text);
                textEditor.setText("");
            }
        }
    };
...
export class LogEntry {
    static fromEngineMessage(engineMessage) {
        return new LogEntry(engineMessage.level, engineMessage.message);
    }
...
class LogConsoleTab {
    constructor(url, title, icon, active, first, logEntries) {
...
export class LogConsole {
    ...
    handleEngineDetected(host, port) {
        ...
        if (tab == null) {
            ...
            tab = new LogConsoleTab(url, title, icon, active, first, []);
    ...
    set visible(value) {
        ...
        if (value) {
            ...
            const handleCommandLineTextEntry = this.handleCommandLineTextEntry.bind(this);
            const handleTabMouseDown = this.handleTabElementMouseDown.bind(this);
            const {root, tabBar, list} = createDomElements(handleCommandLineTextEntry);
